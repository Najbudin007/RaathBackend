            ...
            <div class="box-content">
                <div class="clearfix bg-lighter">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h3 class="mb-0 text-uppercase">portfoliocategory List</h3>
                        <a href="{{ route('admin.portfolio_categories.create') }}" class="btn btn-primary"> <i
                ...
                <table id="scroll-horizontal-datatable" class="table table-responsive table-striped table-bordered display"
                    ...
                    <tbody>
                        @forelse ($portfoliocategories as $key=>$portfoliocategory)
                            <tr id="row_{{ $key }}">
                                <td>{{ $portfoliocategory->title }}</td>
                                <td><span class="badge bg-{{ $portfoliocategory->status == 1 ? 'success' : 'danger' }}">
                                        {{ $portfoliocategory->status == 1 ? 'Active' : 'Inactive' }}
                                    </span></td>

                                <td>
                                    <a href="{{ route('admin.portfolio_categories.edit', $portfoliocategory->id) }}"
                                        class="btn btn-primary btn-sm">Edit</a>
                                    <button type="button"
                                        onclick="confirmDelete('{{ route('admin.portfolio_categories.destroy', $portfoliocategory->id) }}', {{ $key }},'{{ csrf_token() }}')"
