            ...
            <div class="box-content">
                <div class="clearfix bg-lighter">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h3 class="mb-0 text-uppercase">Book List</h3>
                    ...
                    <tbody>
                        @forelse ($books as $key=>$book)
                            <tr id="row_{{ $key }}">
                                ...
                                <td><span class="badge bg-{{ $book->status == 1 ? 'success' : 'danger' }}">
                                        {{ $book->status == 1 ? 'Active' : 'Inactive' }}
                                    </span></td>
                                <td><img class="img-fluid" src="{{ Str::storage_path($book->image) }}"
                                        style="height: 100px; width:200px"></td>
                                {{-- <td>{{$book->description }}</td> --}}

                                <td>
                                    <a href="{{ route('admin.services.edit', $book->id) }}"
                                        class="btn btn-primary btn-sm">Edit</a>
                                    <button type="button"
                                        onclick="confirmDelete('{{ route('admin.services.destroy', $book->id) }}', {{ $key }},'{{ csrf_token() }}')"
