            ...
            <div class="box-content">
                <div class="clearfix bg-lighter">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h3 class="mb-0 text-uppercase">team List</h3>
                        <a href="{{ route('admin.teams.create') }}" class="btn btn-primary"> <i class="ri-add-box-line"></i>
                            Add
                            New</a>
                    </div>
                </div>
                    ...
                    <tbody>
                        @forelse ($teams as $key=>$team)
                            <tr id="row_{{ $key }}">
                                ...
                                <td><span class="badge bg-{{ $team->status == 1 ? 'success' : 'danger' }}">
                                        {{ $team->status == 1 ? 'Active' : 'Inactive' }}
                                    </span></td>
                                <td><img src="{{ Str::storage_path($team->image) }}" style="height: 100px"></td>
                                <td>{{ $team->email }}</td>
                                <td>{{ $team->facebook }}</td>
                                <td>{{ $team->linkedin }}</td>
                                <td>{{ $team->website }}</td>
                                ...
                                <td>
                                    ...
                                    <button type="button"
                                        onclick="confirmDelete('{{ route('admin.teams.destroy', $team->id) }}', {{ $key }},'{{ csrf_token() }}')"
