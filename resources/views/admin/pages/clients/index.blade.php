            ...
            <div class="box-content">
                <div class="clearfix bg-lighter">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h3 class="mb-0 text-uppercase">client List</h3>
                ...
                <table id="scroll-horizontal-datatable" class="table table-responsive table-striped table-bordered display"
                    ...
                    <thead>
                        <tr>
                            ...
                            <th>{{ ucwords(str_replace('_', ' ', 'icon')) }}</th>
                            <th>{{ ucwords(str_replace('_', ' ', 'logo')) }}</th>
                            <th>{{ ucwords(str_replace('_', ' ', 'description')) }}</th>
                            <th>{{ ucwords(str_replace('_', ' ', 'rating')) }}</th>
                            <th>{{ ucwords(str_replace('_', ' ', 'url')) }}</th>
                    ...
                    <tbody>
                        @forelse ($clients as $key=>$client)
                            <tr id="row_{{ $key }}">
                                ...
                                <td><span class="badge bg-{{ $client->status == 1 ? 'success' : 'danger' }}">
                                {{ $client->status == 1 ? 'Active' : 'Inactive' }}
                                </span></td>
                                <td><img class="img-fluid" src="{{ Str::storage_path($client->icon) }}" style="height: 100px; width:200px"></td>
                                <td><img class="img-fluid" src="{{ Str::storage_path($client->logo) }}" style="height: 100px; width:200px"></td>
                                ...
                                <td>
                                    ...
                                    <button type="button"
                                        onclick="confirmDelete('{{ route('admin.clients.destroy', $client->id) }}', {{ $key }},'{{ csrf_token() }}')"
