            ...
            <div class="box-content">
                <div class="clearfix bg-lighter">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h3 class="mb-0 text-uppercase">service List</h3>
                ...
                <table id="scroll-horizontal-datatable" class="table table-responsive table-striped table-bordered display"
                    ...
                    <thead>
                        <tr>
                            ...
                            <th>Category</th>
                            <th>{{ ucwords(str_replace('_', ' ', 'sub_title')) }}</th>
                    ...
                    <tbody>
                        @forelse ($services as $key=>$service)
                            <tr id="row_{{ $key }}">
                                ...
                                <td><span class="badge bg-{{ $service->status == 1 ? 'success' : 'danger' }}">
                                        {{ $service->status == 1 ? 'Active' : 'Inactive' }}
                                    </span></td>
                                <td><img class="img-fluid" src="{{ Str::storage_path($service->icon) }}" style="height: 100px; width:200px"></td>
                                ...
                                <td>
                                    ...
                                    <button type="button"
                                        onclick="confirmDelete('{{ route('admin.services.destroy', $service->id) }}', {{ $key }},'{{ csrf_token() }}')"
