            ...
            <div class="box-content">
                <div class="clearfix bg-lighter">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h3 class="mb-0 text-uppercase">career List</h3>
                        <a href="{{ route('admin.careers.create') }}" class="btn btn-primary"> <i
                ...
                <table id="scroll-horizontal-datatable" class="table table-responsive table-striped table-bordered display"
                    ...
                    <thead>
                        <tr>
                            ...
                            <th>{{ ucwords(str_replace('_', ' ', 'position')) }}</th>
                            <th>{{ ucwords(str_replace('_', ' ', 'status')) }}</th>
                            <th>{{ ucwords(str_replace('_', ' ', 'expired_date')) }}</th>
                    ...
                    <tfoot>
                        <tr>
                            <th>{{ ucwords(str_replace('_', ' ', 'title')) }}</th>
                            <th>{{ ucwords(str_replace('_', ' ', 'position_id')) }}</th>
                            <th>{{ ucwords(str_replace('_', ' ', 'status')) }}</th>
                            <th>{{ ucwords(str_replace('_', ' ', 'expired_date')) }}</th>
                            <th>{{ ucwords(str_replace('_', ' ', 'image')) }}</th>
                    ...
                    <tbody>
                        @forelse ($careers as $key=>$career)
                            <tr id="row_{{ $key }}">
                                <td>{{ $career->title }}</td>
                                <td>{{ $career->role->title ?? '-' }}</td>
                                <td><span class="badge bg-{{ $career->status == 1 ? 'success' : 'danger' }}">
                                        {{ $career->status == 1 ? 'Active' : 'Inactive' }}
                                    </span></td>
                                <td>{{ $career->expired_date }}</td>
                                <td><img src="{{ Str::storage_path($career->image) }}" style="height: 100px"></td>

                                <td>
                                    <a href="{{ route('admin.careers.edit', $career->id) }}"
                                        class="btn btn-primary btn-sm">Edit</a>
                                    <button type="button"
                                        onclick="confirmDelete('{{ route('admin.careers.destroy', $career->id) }}', {{ $key }},'{{ csrf_token() }}')"
