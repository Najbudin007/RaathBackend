        ...
        <form method="POST" action="{{ $action }}" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input id="title" type="text" class="form-control @error('title') is-invalid @enderror"
                    name="title" required value="{{ old('title', $serviceCategory->title ?? '') }}">
                @error('title')
            ...
            <div class="mb-3">
                ...
                <input id="sub_title" type="text" class="form-control @error('sub_title') is-invalid @enderror"
                    name="sub_title" required value="{{ old('sub_title', $serviceCategory->sub_title ?? '') }}">
                @error('sub_title')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
            ...
            <div class="mb-3">
                ...
                <select id="sts" name="status" class="form-select" required>
                    ...
                    @foreach ($statusOptions as $name => $value)
                        <option value="{{ $value }}"
                            {{ (isset($serviceCategory) && $serviceCategory->status == $value) || old('status') == $value ? 'selected' : '' }}>
                            {{ $name }}</option>
                    @endforeach
                </select>
                @error('status')
            ...
            <div class="mb-3 row">
                ...
                <div id="" class="form-text text-danger">{{ Str::img_size('serviceCategorys') }}</div>
                @error('icon')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
                ...
                <div class="col-sm-auto">
                    <div id="post-img">
                        @if (isset($serviceCategory) && $serviceCategory->icon)
                            <img src="{{ Str::storage_path($serviceCategory->icon) }}" height="200px" width="auto">
                        @else
            ...
            <div class="mb-3">
                ...
                <textarea id="description" name="description" class="form-control required">{{ old('description', $serviceCategory->description ?? '') }}</textarea>
                @error('description')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
