            ...
            <div class="box-content">
                <div class="clearfix bg-lighter">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h3 class="mb-0 text-uppercase">servicecategory List</h3>
                        <a href="{{ route('admin.service_categories.create') }}" class="btn btn-primary"> <i
                    ...
                    <tbody>
                        @forelse ($servicecategories as $key=>$servicecategory)
                            <tr id="row_{{ $key }}">
                                ...
                                <td><span class="badge bg-{{ $servicecategory->status == 1 ? 'success' : 'danger' }}">
                                        {{ $servicecategory->status == 1 ? 'Active' : 'Inactive' }}
                                    </span></td>
                                <td><img class="img-fluid" src="{{ Str::storage_path($servicecategory->icon) }}" style="height: 100px; width:200px"></td>
                                <td>
                                    <a href="{{ route('admin.service_categories.edit', $servicecategory->id) }}"
                                        class="btn btn-primary btn-sm">Edit</a>
                                    <button type="button"
                                        onclick="confirmDelete('{{ route('admin.service_categories.destroy', $servicecategory->id) }}', {{ $key }},'{{ csrf_token() }}')"
